...
import { Box, Card, CardContent, TextField, Dialog, Chip, Typography, DialogActions, Button, DialogTitle, DialogContent, Tabs, Tab, IconButton, Alert, Stack, useTheme, useMediaQuery, Pagination, Skeleton, alpha, List, ListItem, ListItemText, ListItemButton, Menu, MenuItem, Divider, Paper } from '@mui/material';
import { Edit as EditIcon, Add as AddIcon, ArrowBack as ArrowBackIcon, Search as SearchIcon, TrendingUp as TrendingUpIcon, Assignment as AssignmentIcon, Schedule as ScheduleIcon, Delete as DeleteIcon, MoreVert as MoreVertIcon, LibraryBooks as LibraryBooksIcon, Folder as FolderIcon, Visibility as VisibilityIcon } from '@mui/icons-material';
...
const TemplateBuilder: React.FC = () => {
  ...
  const handleSaveEditVisitType = () => {
    if (selectedMenuVisitType && editingVisitTypeName.trim()) {
      setVisitTypes(prev => prev.map(vt => vt.id === selectedMenuVisitType.id ? {
        ...vt,
        name: editingVisitTypeName.trim().toUpperCase()
      } : vt));
  ...
  const handleSaveEditTemplate = () => {
    if (selectedMenuTemplate && editingTemplateName.trim()) {
      setTemplates(prev => prev.map(t => t.id === selectedMenuTemplate.id ? {
        ...t,
  ...
  const filteredTemplates = templates.filter(template => {
    const matchesSearch = template.title.toLowerCase().includes(searchTerm.toLowerCase()) || template.specialty.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const renderVisitTypes = () => <Box>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={4}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpenAddType(true)} sx={{
      ...
      borderRadius: 3
        ...
        p: 0
          ...
          {visitTypes.map((visitType, index) => <React.Fragment key={visitType.id}>
              <ListItem disablePadding>
                <ListItemButton onClick={() => handleVisitTypeSelect(visitType)} sx={{
              py: 3,
              px: 3,
              '&:hover': {
                backgroundColor: alpha(bravoColors.primaryFlat, 0.05)
                  ...
                  fontSize: '1.1rem'
                        ...
                        <IconButton size="small" onClick={e => handleVisitTypeMenuClick(e, visitType)} sx={{
                  color: bravoColors.primaryFlat
                }}>
                          <MoreVertIcon />
                        </IconButton>
              ...
              {index < visitTypes.length - 1 && <Divider sx={{
            mx: 3
          }} />}
            </React.Fragment>)}
        </List>
  ...
  const renderTemplates = () => <Box>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={4}>
        <Box sx={{
        flex: 1
      }}>
          <Box display="flex" alignItems="center" gap={2} mb={2}>
            <IconButton onClick={handleBackToVisitTypes} sx={{
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpenCreateTemplate(true)} sx={{
        backgroundColor: bravoColors.secondary,
        color: 'white',
        borderRadius: 3,
        px: 4,
      ...
      <TextField fullWidth placeholder="Search templates by name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} InputProps={{
      ...
      {isLoading ? <Box>
        ...
        borderRadius: 2
          ...
          p: 3
                ...
                <Skeleton variant="text" height={24} sx={{
            mb: 1
          }} />
                <Skeleton variant="text" height={20} width="60%" />
              </CardContent>
            </Card>)}
        </Box> : paginatedTemplates.length > 0 ? <Paper elevation={1} sx={{
      borderRadius: 3
    }}>
          <List sx={{
        p: 0
      }}>
            {paginatedTemplates.map((template, index) => <React.Fragment key={template.id}>
            ...
            '&:hover': {
              '& .template-action-buttons': {
                ...
                transform: 'translateX(0)'
                  ...
                  <ListItemButton onClick={() => handleTemplateView(template)} sx={{
              py: 2,
              px: 3,
              '&:hover': {
                backgroundColor: alpha(bravoColors.primaryFlat, 0.05)
                ...
                flex: 1
                  ...
                  flex: 1
                    ...
                    mt: 0.5
                      ...
                      <Box className="template-action-buttons" display="flex" alignItems="center" gap={1} sx={{
                  opacity: 0,
                  transform: 'translateX(10px)',
                  transition: 'all 0.2s ease-in-out'
                }}>
                        <Button variant="outlined" size="small" startIcon={<VisibilityIcon />} onClick={e => {
                    e.stopPropagation();
                    ...
                    '&:hover': {
                      ...
                      color: bravoColors.primaryFlat
                        ...
                        <Button variant="contained" size="small" startIcon={<EditIcon />} onClick={e => {
                    e.stopPropagation();
                    ...
                    '&:hover': {
                      backgroundColor: bravoColors.primaryDark
                        ...
                        <IconButton onClick={e => handleTemplateMenuClick(e, template)} sx={{
                    color: bravoColors.primaryFlat
                  }}>
                          <MoreVertIcon />
                        </IconButton>
                ...
                {index < paginatedTemplates.length - 1 && <Divider sx={{
            mx: 3
          }} />}
              </React.Fragment>)}
          </List>
      ...
      {totalPages > 1 && <Box display="flex" justifyContent="center" mt={4}>
          <Pagination count={totalPages} page={currentPage} onChange={(event, page) => setCurrentPage(page)} color="primary" size="large" sx={{
  ...
  const renderTemplateView = () => <Box>
      <Box display="flex" alignItems="center" mb={3}>
        <IconButton onClick={handleBackToTemplates} sx={{
        mr: 2,
        color: bravoColors.primaryFlat
      }}>
          <ArrowBackIcon />
      ...
      borderRadius: 2
        ...
        borderRadius: 2
          ...
          <Box display="flex" gap={2} mt={3}>
            <Button variant="contained" startIcon={<EditIcon />} onClick={() => handleTemplateEdit(viewingTemplate)} sx={{
          backgroundColor: bravoColors.primaryFlat,
          color: 'white',
          borderRadius: 2,
          px: 3,
  ...
  const renderMyTemplatesContent = () => {
    if (currentScreen === 'templateEditor' && editingTemplate) {
      return <Box>
          ...
          <TemplateEditor initialItems={sectionList} onSave={handleSaveTemplate} />
        </Box>;
    }
  ...
  return <Box sx={{
    p: {
      ...
      mb: 4
        ...
        <Tabs value={currentTab} onChange={(event, newValue) => setCurrentTab(newValue)} sx={{
        '& .MuiTab-root': {
          textTransform: 'none',
          fontWeight: 600,
          fontSize: '1.1rem',
        ...
        '& .MuiTabs-indicator': {
          ...
          <Tab icon={<FolderIcon sx={{
          fontSize: 20
        }} />} iconPosition="start" label="My Templates" />
          <Tab icon={<LibraryBooksIcon sx={{
          fontSize: 20
        }} />} iconPosition="start" label="Template Library" />
        </Tabs>
      </Box>

      {/* Tab Content */}
      {currentTab === 0 && renderMyTemplatesContent()}
      {currentTab === 1 && <TemplateLibraryTab visitTypes={visitTypes.map(vt => vt.name)} onAddTemplate={handleAddLibraryTemplate} />}

      {/* Enhanced Template Creation Dialog */}
      <ImprovedTemplateCreationDialog open={openCreateTemplate} onClose={() => {
      ...
      <Dialog open={openAddType} onClose={() => setOpenAddType(false)} maxWidth="sm" fullWidth PaperProps={{
      sx: {
        borderRadius: 3,
        minHeight: '200px'
      }
        ...
        py: 3
          ...
          <TextField autoFocus margin="dense" label="Visit Type Name" fullWidth variant="outlined" value={newTypeName} onChange={e => setNewTypeName(e.target.value)} placeholder="e.g., Progress Note, Discharge Summary" sx={{
          '& .MuiOutlinedInput-root': {
            borderRadius: 2,
            '&:hover': {
              '& .MuiOutlinedInput-notchedOutline': {
        ...
        pb: 3
          ...
          <Button onClick={() => setOpenAddType(false)} variant="outlined" sx={{
          borderRadius: 2,
          borderColor: bravoColors.primaryFlat,
          color: bravoColors.primaryFlat,
          px: 3
          ...
          <Button onClick={handleAddType} variant="contained" disabled={!newTypeName.trim()} sx={{
          borderRadius: 2,
          backgroundColor: bravoColors.primaryFlat,
          px: 3,
          '&:hover': {
      ...
      {/* Visit Type Menu */}
      <Menu anchorEl={visitTypeMenuAnchor} open={Boolean(visitTypeMenuAnchor)} onClose={handleMenuClose} PaperProps={{
      sx: {
        borderRadius: 2,
        minWidth: 150,
        ...
        <MenuItem onClick={handleEditVisitType} sx={{
        py: 1.5
      }}>
          <EditIcon sx={{
          mr: 2,
        ...
        <MenuItem onClick={handleDeleteVisitType} sx={{
        py: 1.5,
        color: 'error.main'
      }}>
          <DeleteIcon sx={{
      ...
      {/* Template Menu */}
      <Menu anchorEl={templateMenuAnchor} open={Boolean(templateMenuAnchor)} onClose={handleMenuClose} PaperProps={{
      sx: {
        borderRadius: 2,
        minWidth: 150,
        boxShadow: '0 4px 20px rgba(0,0,0,0.15)'
        ...
        <MenuItem onClick={handleEditTemplate} sx={{
        py: 1.5
      }}>
          <EditIcon sx={{
          mr: 2,
        ...
        <MenuItem onClick={handleDeleteTemplate} sx={{
        py: 1.5,
        color: 'error.main'
      }}>
          <DeleteIcon sx={{
      ...
      <Dialog open={editVisitTypeDialog} onClose={() => setEditVisitTypeDialog(false)} maxWidth="sm" fullWidth PaperProps={{
      sx: {
        borderRadius: 3,
        minHeight: '200px'
      }
        ...
        py: 3
          ...
          <TextField autoFocus margin="dense" label="Visit Type Name" fullWidth variant="outlined" value={editingVisitTypeName} onChange={e => setEditingVisitTypeName(e.target.value)} sx={{
          '& .MuiOutlinedInput-root': {
            borderRadius: 2,
            '&:hover': {
              '& .MuiOutlinedInput-notchedOutline': {
        ...
        pb: 3
          ...
          <Button onClick={() => setEditVisitTypeDialog(false)} variant="outlined" sx={{
          borderRadius: 2,
          borderColor: bravoColors.primaryFlat,
          color: bravoColors.primaryFlat,
          px: 3
          ...
          <Button onClick={handleSaveEditVisitType} variant="contained" disabled={!editingVisitTypeName.trim()} sx={{
          borderRadius: 2,
          backgroundColor: bravoColors.primaryFlat,
          px: 3,
          '&:hover': {
      ...
      <Dialog open={editTemplateDialog} onClose={() => setEditTemplateDialog(false)} maxWidth="sm" fullWidth PaperProps={{
      sx: {
        borderRadius: 3,
        minHeight: '200px'
      }
        ...
        py: 3
          ...
          <TextField autoFocus margin="dense" label="Template Name" fullWidth variant="outlined" value={editingTemplateName} onChange={e => setEditingTemplateName(e.target.value)} sx={{
          '& .MuiOutlinedInput-root': {
            borderRadius: 2,
            '&:hover': {
              '& .MuiOutlinedInput-notchedOutline': {
        ...
        pb: 3
          ...
          <Button onClick={() => setEditTemplateDialog(false)} variant="outlined" sx={{
          borderRadius: 2,
          borderColor: bravoColors.primaryFlat,
          color: bravoColors.primaryFlat,
          px: 3
          ...
          <Button onClick={handleSaveEditTemplate} variant="contained" disabled={!editingTemplateName.trim()} sx={{
          borderRadius: 2,
          backgroundColor: bravoColors.primaryFlat,
          px: 3,
          '&:hover': {
